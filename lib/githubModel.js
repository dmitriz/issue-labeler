...
async function callGithubModel({ prompt }) {
  try {
    ...
    if (error.response?.status === 429) {
      console.warn("Rate limit exceeded. Consider implementing retry with exponential backoff.");
    }
    ...
    throw new Error(`GitHub Models API request failed: ${JSON.stringify(errorDetails)}`);
  }
  }
}
